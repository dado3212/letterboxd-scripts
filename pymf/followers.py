...
for unwatched_liker in unwatched_likers:
  ...
  if info['reviewLikes'] > 2000 and info['watches'] > 100 and info['diaryEntriesThisYear'] > 5:
    if followable(unwatched_liker['id']):
      print('https://boxd.it/' + unwatched_liker['id'] + ': ' + unwatched_liker['name'] + ' [' + ('Watched' if unwatched_liker['watched'] else 'Unwatched') + '] - ' + str(info['reviewLikes']) + ' - ' + str(round(info['following'] * 100.0 / info['followers']) / 100))
